 ...
 *
 * Copyright (C) 2011 - 2015 Paul Terry.
 *
                               ...
                               gboolean preview)
{
  ...
  guchar *drawable_inrow, *drawable_outrow;
...
static void sdp_noise_reduction_changed(GtkAdjustment *adj, void *up)
{
  ...
  sdc->noise_reduction = (int) ((gtk_adjustment_get_value(adj) * 255.0) / 100.0);
...
static void sdp_brightness_changed(GtkAdjustment *adj, void *up)
{
  ...
  sdc->brightness = (int) gtk_adjustment_get_value(adj);
 ...
 * Help dialog box.
 */
static void sdp_help_dialog(const gchar *help_id, gpointer help_data)
{
...
{
  GtkWidget *dialog, *dialog_vbox, *main_vbox, *frame_1, *frame_2, *frame_3;
  GtkWidget *preview;
  GtkWidget *table_1, *table_2, *table_3;
  ...
  gtk_container_add(GTK_CONTAINER(dialog_vbox), main_vbox);
   ...
   */
  primary_layer_combo_box = (GtkWidget *) gtk_combo_box_new_text();
  gtk_combo_box_insert_text(GTK_COMBO_BOX(primary_layer_combo_box), 0, "Unfiltered Layer");
  for (i = 0; i < n_layers; i++) {
    layer_name = (gchar *) gimp_drawable_get_name(layer_ids[i]);
   ...
   */
  subtraction_layer_combo_box = (GtkWidget *) gtk_combo_box_new_text();
